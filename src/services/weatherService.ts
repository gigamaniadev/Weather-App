...
const fetchWithCache = async (url: string, duration = CACHE_DURATION.DEFAULT) => {
...
const CITIES_BY_REGION = {
  ...
  OCEANIA: [
    ...
    { name: 'Brisbane', country: 'AU', lat: -27.4705, lon: 153.0260 }
  ],
  SOUTH_AMERICA: [
...
const getRegionFromCoordinates = (lat: number, lon: number): keyof typeof CITIES_BY_REGION => {
...
export async function fetchMajorCitiesWeather(userLat?: number, userLon?: number) {
  let selectedCities = [];
  ...
  const promises = selectedCities.map(city => 
    fetchWeatherData(city.lat, city.lon, CACHE_DURATION.MAJOR_CITIES)
      .then(data => ({
        city: city.name,
...
export async function fetchWeatherData(latitude: number, longitude: number, cacheDuration = CACHE_DURATION.DEFAULT) {
...
export async function fetchTomorrowWeather(latitude: number, longitude: number) {
...
export async function fetchWeeklyForecast(latitude: number, longitude: number) {
  ...
  const dailyForecasts = data.list.reduce((acc: any, forecast: any) => {
    ...
    if (!acc[dateKey]) {
      ...
      acc[dateKey].temp_max = Math.max(acc[dateKey].temp_max, forecast.main.temp);
      acc[dateKey].temp_min = Math.min(acc[dateKey].temp_min, forecast.main.temp);
    }
